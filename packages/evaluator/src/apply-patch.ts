  ...
  return new Promise((resolve, reject) => {
    ...
    if (input !== undefined) {
      child.stdin.on("error", () => {
        ...
        // The close event will capture the actual exit code.
      });
...
export async function applyPatch(
  ...
  const applyResult = await runGitCommand(workspacePath, ["apply", "--verbose", "-"], patchContent);
  ...
  const diffResult = await runGitCommand(workspacePath, ["diff", "--no-ext-diff", "--", "."]);
