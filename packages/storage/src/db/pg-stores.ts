    ...
    return {
      ...
      `;
        ...
        async listAll(pagination?: PaginationParams): Promise<ReadonlyArray<Repository>> {
            ...
            const rows = await sql`SELECT * FROM repositories ORDER BY created_at DESC LIMIT ${limit} OFFSET ${offset}`;
...
export function createSuiteStore(sql: Sql): SuiteStore {
    return {
        ...
        async findByRepository(repositoryId: string, pagination?: PaginationParams): Promise<ReadonlyArray<BenchmarkSuite>> {
            const limit = pagination?.limit ?? DEFAULT_PAGE_LIMIT;
            const offset = pagination?.offset ?? 0;
            const rows =
    ...
    return {
      ...
      `;
        ...
        async listAll(pagination?: PaginationParams): Promise<ReadonlyArray<AgentProfile>> {
            const limit = pagination?.limit ?? DEFAULT_PAGE_LIMIT;
            const offset = pagination?.offset ?? 0;
            const rows = await sql`SELECT * FROM agent_profiles ORDER BY created_at DESC LIMIT ${limit} OFFSET ${offset}`;
            return rows.map((r) => toAgentProfile(r as Record<string, unknown>));
        },
    };
}
