...
export async function runCommand(args: ReadonlyArray<string>): Promise<void> {
    ...
    if (suiteId === undefined) {
        throw new Error("--suite is required. Usage: repobench run --suite <suiteId> --agent <agentProfileId>");
    }

    if (agentProfileId === undefined) {
        throw new Error("--agent is required. Usage: repobench run --suite <suiteId> --agent <agentProfileId>");
    }
