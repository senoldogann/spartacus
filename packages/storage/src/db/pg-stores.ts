...
export function createRepositoryStore(sql: Sql): RepositoryStore {
  return {
    ...
    async listAll(): Promise<ReadonlyArray<Repository>> {
      const rows = await sql`SELECT * FROM repositories ORDER BY created_at DESC`;
...
export function createSuiteStore(sql: Sql): SuiteStore {
  return {
    ...
    async findByRepository(repositoryId: string): Promise<ReadonlyArray<BenchmarkSuite>> {
      const rows =
        await sql`SELECT * FROM benchmark_suites WHERE repository_id = ${repositoryId} ORDER BY created_at DESC`;
...
export function createRunStore(sql: Sql): RunStore {
  return {
    ...
    async findBySuite(suiteId: string): Promise<ReadonlyArray<Run>> {
      const rows =
        await sql`SELECT * FROM runs WHERE suite_id = ${suiteId} ORDER BY created_at DESC`;
...
export function createAgentProfileStore(sql: Sql): AgentProfileStore {
  return {
    ...
    async listAll(): Promise<ReadonlyArray<AgentProfile>> {
      const rows = await sql`SELECT * FROM agent_profiles ORDER BY created_at DESC`;
      return rows.map((r) => toAgentProfile(r as Record<string, unknown>));
    },
  };
}
