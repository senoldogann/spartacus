...
export async function importCommand(args: ReadonlyArray<string>): Promise<void> {
  ...
  if (repoArg === undefined) {
    throw new Error(
      "--repo is required. Usage: repobench import --repo owner/name --test-command 'npm test'",
    );
  }
  ...
  if (testCommand === undefined) {
    throw new Error(
      "--test-command is required. Usage: repobench import --repo owner/name --test-command 'npm test'",
    );
  }
